...
for idx, fname in enumerate(file_list_knee):
    ...
    try:
        ...
        # Normalize
        zf_coil_img = fft.fftshift(fft.ifft2(fft.fftshift(tstDsKsp, axes=(0, 1)), axes=(0, 1), norm='ortho'),
                                   axes=(0, 1))
        ...
        im_rec = axes_comp[0].imshow(recon_restored, cmap='gray', vmin=0,
        ...
        im_gt = axes_comp[1].imshow(gt_restored, cmap='gray', vmin=0, vmax=1 if vol_max is None else gt_restored.max())
